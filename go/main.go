...
func postInitialize(c echo.Context) error {
	...
	for _, image := range images {
		if err := WriteFile(getIconPath(image.JIAUserID, image.JIAIsuUUID), image.Image); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
	}
...
func generateIsuGraphResponse(tx *sqlx.Tx, jiaIsuUUID string, graphDate time.Time) ([]GraphResponse, error) {
	...
	rows, err := tx.Queryx("SELECT * FROM `isu_condition` WHERE `jia_isu_uuid` = ? AND ? <= timestamp AND timestamp <= ? ORDER BY `timestamp` ASC", jiaIsuUUID, graphDate, graphDate.Add(24*time.Hour))
	if err != nil {
...
func postIsuCondition(c echo.Context) error {
	...
	err = bulkinsert01(c, tx, req, jiaIsuUUID)
	if err != nil {
		return err
	}
